...
namespace Day07_02 {
    ...
    struct Card {
        ...
        static const constexpr std::uint8_t JokerValue = 1;
    ...
    struct Hand {
        ...
        HandType getHandType() const {
            ...
            std::uint8_t jokerAmount = 0;
            if (frequencies.contains(Card::JokerValue)) {
                jokerAmount = frequencies.at(Card::JokerValue);
            }

            if (jokerAmount != 0) {
                frequencies.erase(Card::JokerValue);
                std::uint8_t highest_key = 0;
                ...
                frequencies.insert_or_assign(highest_key, max_val + jokerAmount);
            }
...
struct AoCDay07 : AoCDay {
    ...
    ResultType solvePart1(const std::string& input, [[maybe_unused]] const bool is_example) const override {
        ...
        std::vector<std::pair<Day07_01::Hand, ResultType>> parsed{};
    ...
    ResultType solvePart2(const std::string& input, [[maybe_unused]] const bool is_example) const override {
        ...
        std::vector<std::pair<Day07_02::Hand, ResultType>> parsed{};
...
DayRegister<AoCDay07> day07{ Input::SameInput("input.txt") >> Input::SameExample("example.txt", 6440, 5905) };
