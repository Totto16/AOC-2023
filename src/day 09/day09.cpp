...
struct AoCDay09 : AoCDay {
    ...
    ResultType solvePart2(const std::string& input, [[maybe_unused]] const bool is_example) const override {
        ...
        for (const auto& line : splitByNewLine(input)) {
            ...
            const auto& start_numbers_vec = Day09::parse_numbers(line);
            std::deque<ResultType> start_numbers{};
            for (const auto& v : start_numbers_vec) {
                start_numbers.push_back(v);
            }

            std::vector<std::deque<ResultType>> numbers{ start_numbers };
            for (std::size_t i = 0; i < numbers.size(); ++i) {
                ...
                if (!have_all_0) {
                    ...
                    std::deque<ResultType> difference{};
                    for (std::size_t j = 1; j < current_numbers.size(); ++j) {
                        difference.push_back(current_numbers.at(j) - current_numbers.at(j - 1));
                    }
            ...
            numbers.back().push_front(0);

            for (std::size_t i = numbers.size() - 1; i != 0; --i) {

                const auto new_element = numbers.at(i - 1).front() - numbers.at(i).front();

                numbers.at(i - 1).push_front(new_element);
            }

            result += numbers.front().front();
        }
...
DayRegister<AoCDay09> day09{ Input::SameInput("input.txt") >> Input::SameExample("example.txt", 114, 2) };
