...
struct std::formatter<std::chrono::nanoseconds, CharT>
  ...
  constexpr auto parse(std::format_parse_context &ctx) {
      ...
      if (*pos == ':') {
        ...
        if (array_idx >= 2) {
          throw std::format_error(
              "Format specifier can only have two specifiers!");
        }
        ++pos;
        continue;
      }
    ...
    for (const auto &c : specifier.at(0)) {
      ...
      default:
        throw std::format_error(
            "Unknown Format specifier in first section: allowed are 'cClL'");
      }
    }
    ...
    if (!specifier.at(1).empty()) {
      if (specifier.at(1).size() != 1) {
        throw std::format_error(
            "Only one digit is allowed in the precision specifier");
      }
      ...
      if (digit < '1' || digit > '6') {
        throw std::format_error(
            "Only digits from 1-6 are allowed in the precision specifier!");
      }
  ...
  auto format(const std::chrono::nanoseconds &nanoseconds,
              std::format_context &ctx) const {

    return std::format_to(ctx.out(), "{}",
                          internals::to_string(nanoseconds, options));
  }
};

template <typename T>
T measure(const std::string &name, std::function<T()> execute) {
  std::cout << std::format("{}measuring {}'{}{}{}'{}:{}\n",
                           ForegroundColor::Blue, ForegroundColor::Cyan,
                           ForegroundColor::Green, name, ForegroundColor::Cyan,
                           ForegroundColor::Blue, Color::reset());
  ...
  std::cout << std::format("{}It took: {}{:cL:3}{}\n", ForegroundColor::Yellow,
                           ForegroundColor::Cyan, duration, Color::reset());
  return result;
}
