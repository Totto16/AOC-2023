...
struct InputDescription {
    ...
    InputDescription(
        ...
        : input_1_name{ input_1_name },
          input_2_name{ input_2_name },
          example_1{ example_1 },
          example_2{ example_2 } {
        //
    }
...
struct InputDescriptionGeneratorMultiple {
    ...
    InputDescriptionGeneratorMultiple(const std::initializer_list<InputDescriptionGeneratorSingle>& list)
        : all{ list } {
        //
    }
...
private:
    ...
    helpers::expected<SuccessResult, std::string> executeExample(internals::Part part, Example example) {
        ...
        try {
            ...
            const auto result =
                    num == 1 ? this->solvePart1(read_file.value(), true) : this->solvePart2(read_file.value(), true);

            if (result == example.result) {
                std::cout << std::format(
                        "{}Solved Example for part {}{}{}{} successfully{}\n", ForegroundColor::Green,
                        Color::color(ForegroundColor::Cyan, Modifier::Bold), num, Color::reset(),
                        ForegroundColor::Green, Color::reset()
                );
            } else {
                ...
                throw std::runtime_error(std::format(
                        "{}expected {}'{}{}{}'{} but "
                        "got {}'{}{}{}'{}!{}",
                        content, brackets, ForegroundColor::Green, example.result, brackets, content, brackets,
                        ForegroundColor::Blue, result, brackets, content, Color::reset()
                ));
            }
    ...
    executePart(internals::Part part, const std::string& name, const bool measureTime) {
        ...
        try {
            ...
            const auto result =
                    measureTime ? measure(
                            std::format(
                                    "{}Day {}{:02}{}{} part {}{}{}", ForegroundColor::Green,
                                    Color::color(ForegroundColor::Cyan, Modifier::Bold), day, Color::reset(),
                                    ForegroundColor::Green, ForegroundColor::Cyan, num, Color::reset()
                            ),
                            execute
                    )
                                : execute();
            ...
            std::cout << std::format(
                    ...
                    Color::color(ForegroundColor::Cyan, Modifier::Bold), num, Color::reset(), ForegroundColor::Blue,
                    ForegroundColor::Cyan, ForegroundColor::Green, result, ForegroundColor::Cyan, Color::reset()
...
public:
    ...
    helpers::expected<SuccessResult, std::string> start(InputDescription description, const bool measureTime) {
        ...
        try {
            ...
            ~executePart(internals::Part::Part1, description.input_1_name, measureTime);

            if (description.example_2.has_value()) {
                ~executeExample(internals::Part::Part2, description.example_2.value());
            }

            ~executePart(internals::Part::Part2, description.input_2_name, measureTime);
...
namespace internals {
    ...
    struct global_init {
        ...
        static std::shared_ptr<std::unordered_map<DayType, AoCDayStorageType>> available_days();
    };
...
struct DayRegister {
    ...
    DayRegister(InputDescription description) {
        ...
        if (internals::global_init::available_days()->contains(instance->day)) {
            throw std::runtime_error(std::format(
                    "{}Can't register Day {}{:02}{}{}, since it's already registered!{}", ForegroundColor::Red,
                    Color::color(ForegroundColor::Yellow, Modifier::Bold), instance->day, Color::reset(),
                    ForegroundColor::Red, Color::reset()
            ));
        }
