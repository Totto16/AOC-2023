...
namespace Day05 {
    ...
    enum class RangeState : std::uint8_t { FullyInclusive, NoOverlap, ThreeWayOverlap, RightOverlap, LeftOverlap };
    ...
    public:
        ...
        Range map_fully(const Range& input) const {
            ...
            assert_equal_enum(
                    range_state(input), RangeState::FullyInclusive, "only fully mappable, if 'FullyInclusive'"
            );
        ...
        std::pair<Range, std::vector<Range>> map_partly(const Range& input) const {
            ...
            switch (range_state(input)) {
                case RangeState::LeftOverlap:
                    return {
                        Range{ own_start + shift_amount, end + shift_amount },
                        { Range{ start, own_start - 1 } }
                    };

                case RangeState::RightOverlap:
                    return {
                        Range{ start + shift_amount, own_end + shift_amount },
                        { Range{ own_end + 1, end } }
                    };

                case RangeState::ThreeWayOverlap:
                    return {
                        Range{     own_start + shift_amount,    own_end + shift_amount},
...
struct AoCDay05 : AoCDay {
    ...
    ResultType solvePart1(const std::string& input, [[maybe_unused]] const bool is_example) const override {
        ...
        assert_equal<std::size_t>(seed_list.size(), 2u, "seed list has to be the correct length");
        ...
        for (const auto& line : lines) {
            ...
            maps.at(index).add(Day05::MapEntry{ numbers.at(0), numbers.at(1), numbers.at(2) });
        }
...
DayRegister<AoCDay05> day05{ Input::SameInput("input.txt") >> Input::SameExample("example.txt", 35, 46) };
